...
**双线/多 POV 接力钩子的措辞红线**：细纲写"结尾设定和钩子"时，如果本章钩子的功能是"读者比角色多知道一点"（例如双 POV 小说里男主章尾埋一个女主线索），细纲里只描述**戏剧反讽的功能意图**（谁在此刻不知道什么、读者比他多知道什么），不要把它写成可以直接抄进正文的句子。像"他不知道，井那头接住这封信的人六百年后正是他的血脉"这种全知叙述者句式，字面写进细纲很自然，但落到正文时如果被直接照抄，会撞上 narrative-writer 的 Gate F 硬规则"禁止章末预告"（`references/anti-ai-writing.md` 模式 8，`story-setup/references/templates/agents/narrative-writer.md` 明确列为"硬，禁令优先"）——这条规则不分场景一律生效，双 POV 章节也不例外。正确的转译方式：把功能意图放在细纲的"结尾设定"里描述清楚，正文落笔时改写成角色自己限知视角内能感知/怀疑/追问的内容（例如把陈述句改成角色自己的直觉、疑问句，或干脆只留客观动作让读者自己拼因果），不能让叙述者替角色说出他本不可能知道的事。写细纲的人和写正文的人如果是不同的 agent 调用，这条转译规则必须在两边都能查到，不能只让写正文的人临场自己悟。
...
- 更新 `追踪/质检进度.md`（若已部署）对应章节行：三个脚本、元信息扫描、consistency-checker、去AI味独立审查、字数核实、对话密度实测（读 check-ai-patterns.js 的 `dialogue-density-stat` info 输出，不必手写脚本现算）逐列打勾
