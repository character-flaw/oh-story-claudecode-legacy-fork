...
同步上游 v0.7.3–v0.7.4，并在此之上补三类**本 fork 自研**的确定性闸口：跨章连续性守卫、规划记号泄漏拦截、情绪与钩子下限。**已部署项目必须重跑 `/story-setup` 并新开会话**——`agents_version` 由 23 升到 24，本轮改了 24 个部署产物（含四端 hook 核与 agent 模板），不重跑则新的硬拦截一条都不生效。

### 跨章连续性守卫（本轮最要紧）

起因是实际反馈的最严重问题：上一章主角住学校宿舍，下一章骑车从家去学校；上一章主角的娘摆针线摊，下一章地痞掀的摊子变成卖菜的。用最小用例在本工具上复现——两处矛盾 `commit` 成功、`check` 全绿，工具一声没吭。
...
系统扫描本 fork 与上游全部规则行，检出「上游硬要求 → 本 fork 软化」4 类，其中 2 类是同一失效模式：
...
- **v0.7.3 追踪状态改单一权威事务模型**（#283）：`tracking_commit.py` + `tracking-transaction.md`（三个 skill 各一份），`追踪/上下文.md` 由手工模板改为 `_tracking-state.json` 的派生视图，相应删除 `上下文.md.tmpl`。
- **v0.7.3 修 #283 自身留下的六条主流程断链**（#292）：其中「回炉备份被守卫打死」一条直接卡死回炉重写——`原稿_{日期}.md` 备份必然命中章号顺序校验且无解；另有两个只读 agent 被要求跑 Bash、旧项目迁移路径在日更与 story-import 之间来回弹。
- **v0.7.4**：导入作品与对标视图分离（#294）、三项确定性 P1 契约冲突（#295，补回「细纲未标字数时默认 3000 字/章」兜底）、部署端探测自触发（#308）。
- **多端 references_dir 误报**：`target_cli` 含多端时该字段是逗号拼接的多条路径，hook 把整串当单一路径判断必然为假，所有多端部署每次会话都误报「参考资料包缺失」。本 fork 与上游 #308 独立修了同一个 bug，采用上游实现（POSIX `IFS` 分词 + `LC_ALL=C sed`，比 bash 数组更可移植），并补两个多端测试用例——原有测试只覆盖单路径，所以这个 bug 从没被测到。
