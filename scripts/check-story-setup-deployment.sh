...
echo "$stale_previous_out" | grep -q '低于 v24' || fail "session-start did not warn for agents_version 22 stale v24 deployment"
...
assert_grep 'AGENTS_VERSION.*-lt 24|AGENTS_VERSION" -lt 24' "$HOOKS_DIR/session-start.sh" "session-start must warn for agents_version 22 under v24 deployment"
assert_grep 'AGENTS_VERSION.*-gt 24|AGENTS_VERSION" -gt 24' "$HOOKS_DIR/session-start.sh" "session-start must reject a newer agents_version as a downgrade"
assert_grep 'agents_version.*小于 `24`|版本 < 24' "$SKILL_DIR/SKILL.md" "story-setup redeploy branch must treat agents_version 22 as stale"
